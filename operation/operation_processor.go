package operation // nolint: dupl, revive
...
const (
	...
	DuplicationTypePoolAndCurrency DuplicationType = "poolandcurrency"
)
...
func (opr *OperationProcessor) checkDuplication(op state.Processor) error { // nolint:revive
	...
	if len(did) > 0 {
		if _, found := opr.duplicated[did]; found {
			...
			case DuplicationTypePoolAndCurrency:
				return errors.Errorf("duplicated pool and currency id, %q found in proposal", did)
			default:
...
func (opr *OperationProcessor) Close() error {
	...
	if opr.cp != nil && len(opr.fee) > 0 {
		op := feefi.NewFeeOperation(feefi.NewFeeOperationFact(opr.pool.Height(), opr.fee))

		pr := feefi.NewFeeOperationProcessor(opr.cp, op)
