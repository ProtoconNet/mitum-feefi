...
func (hd *Handlers) handleAccountFeefi(w http.ResponseWriter, r *http.Request) {
	var address base.Address
	if a, err := base.DecodeAddressFromString(strings.TrimSpace(mux.Vars(r)["address"]), hd.enc); err != nil {
		HTTP2ProblemWithError(w, err, http.StatusBadRequest)
	...
	if v, err, shared := hd.rg.Do(cachekey, func() (interface{}, error) {
		i, err := hd.handleAccountFeefiInGroup(address, fid)
...
func (hd *Handlers) handleAccountFeefiInGroup(
	address base.Address,
	fid string,
) ([]byte, error) {
	v, err := hd.database.FeefiByAddress(address, fid)
	if err != nil {
		return nil, err
	}
